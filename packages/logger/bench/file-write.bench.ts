...
const LOG_ENTRY =
	JSON.stringify({ timestamp: Date.now(), level: 'INFO', content: { message: 'Hello world' } }) +
	'\n';
...
barplot(() => {
	summary(() =>
		group(`📝 File Write Comparison (${ITERATIONS} writes)`, () => {
			...
			bench('Bun.FileSink (native append)', async () => {
				...
				for (let i = 0; i < ITERATIONS; ++i) do_not_optimize(sink.write(LOG_ENTRY));
			...
			bench('createWriteStream (Node)', async () => {
				...
				for (let i = 0; i < ITERATIONS; ++i) do_not_optimize(stream.write(LOG_ENTRY));
