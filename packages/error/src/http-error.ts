...
export class HttpError<const TCause = unknown> extends AppError<TCause> {
	...
	public constructor(
		message: string,
		httpStatusCode: HttpStatusKey | HttpStatusCode,
		cause?: TCause
	);
	public constructor(message: string, a?: unknown, b?: unknown) {
		const isStatus = typeof a === 'number' || (typeof a === 'string' && Object.hasOwn(HTTP_STATUS_CODES, a));
		const status = isStatus
			? typeof a === 'number'
				? a
				: HTTP_STATUS_CODES[a as keyof typeof HTTP_STATUS_CODES]
			: HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR;

		super(message, (isStatus ? b : a) as TCause);
